 ...
 *
 * Install Grafana, Loki, Alloy and Tempo in k8s cluster using helm chart
 * ----------------------------------------------------------------------
 ...
 *
 Important Notes from my Instructor:
 ---------------------------------------
 Important Update: Bitnami Is No Longer Open Source
In the upcoming lectures, we’ll use Helm Charts to deploy several components such as Kafka, Keycloak, Prometheus, Loki, Alloy, Tempo, and Grafana.

Previously, these charts and images were provided by Bitnami, which was open source and freely available to everyone. However, Bitnami has now transitioned into a commercial product, and enterprise organizations are required to pay thousands of dollars each month to use their production-grade images and Helm Charts.

You can read more about this change in the official announcement below:
👉 How to Prepare for the Bitnami Changes Coming Soon

What We’ll Use Instead
To ensure you can continue your learning seamlessly, I’ve prepared custom Helm Charts that serve the same purpose as Bitnami’s — and they’re ready to use for all the demonstrations in this course.

You can find them in the GitHub repository under the helm-new folder:
🔗 https://github.com/eazybytes/microservices/tree/3.4.1/section_16/helm-new

There’s no need to modify anything inside these charts — they are fully configured for the learning exercises.

⚙️ Deployment Order
Please install the Helm charts in the following order, waiting 1–2 minutes between each deployment:
...
The charts we’ll use are simplified versions, but the concepts and workflows remain exactly the same as you would experience with production-grade Bitnami charts.

⚠️ Important Note
The demo/sample Helm chart discussed in the next lecture is based on Bitnami, which is no longer open source.
So, please don’t install or run that chart — simply watch the lecture to understand the Helm chart concepts and structure.

So, let’s continue our hands-on learning journey — have fun exploring Helm and Kubernetes! 🚀
